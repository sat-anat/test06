...
- 並列取得: JS 資産を 8 並列で取得
- payload(_payload.json) を最優先、その後に JS 資産へフォールバック
"""
...
import requests
import requests_cache  # ★ 追加: HTTP キャッシュ

# -----------------------
# リクエストキャッシュ（1日）
# -----------------------
# .requests_cache/http-cache.sqlite等に保存。Actions 側でこのディレクトリをキャッシュします。
requests_cache.install_cache('.requests_cache/http-cache', expire_after=86400)

# -----------------------
# 正規表現（Nuxt / 資産抽出）
# -----------------------
NUXT_CONFIG_RE = re.compile(r'window\.__NUXT__\.config\s*=\s*\{(?P<body>.*?)\}', re.S)
NUXT_APP_RE = re.compile(
    r'"?app"?\s*:\s*\{[^}]*?\bbaseURL"\s*:\s*"(?P<baseURL>[^"]+)"[^}]*?\bbuildId"\s*:\s*"(?P<buildId>[^"]+)"[^}]*?\bbuildAssetsDir"\s*:\s*"(?P<buildAssetsDir>[^"]+)"',
    re.S,
)
NUXT_DATA_SRC_RE = re.compile(r'id="__NUXT_DATA__"[^>]*\sdata-src="(?P<src>[^"]+_payload\.json[^"]*)"', re.S)
MODULE_HREF_RE = re.compile(r'href="([^"]+/__?nuxt/[^"]+\.js)"')
ALL_JS_PATH_RE = re.compile(r'/__?nuxt/[^"\'\s]+\.js')

# -----------------------
# カード検出ヒント
# -----------------------
SKILL_DSL_HINTS = [
    "ap_up (", "score_up (", "vol_buff (", "score_buff (", "vol_up (",
    "reset ()", "splice ()",
    "appeal_up (", "ap_reduce (", "cooltime_reduce ("
]
MEMBER_HINTS = ["花帆", "さやか", "梢", "綴理", "瑠璃乃", "慈", "吟子", "小鈴", "姫芽", "セラス", "泉"]

HEADERS = [
    "id", "title", "member", "rarity", "ap_cost", "smile", "pure", "cool", "mental",
    "skill_code", "center_skill_code", "center_trait_code", "source_asset", "last_seen"
]
...
def _session() -> requests.Session:
    s = requests.Session()
    s.headers.update({
        "User-Agent": "card-extractor/1.2 (+https://asmape0104.github.io/scshow-calculator/)",
        "Accept": "*/*",
...
def fetch_many(urls: List[str], session: Optional[requests.Session] = None, max_workers: int = 8) -> Dict[str, Optional[str]]:
    """URL 群を並列取得（max_workers=8）。失敗は None。"""
    s = session or _session()
...
# Nuxt 資産・payload の検出
# -----------------------
def _parse_nuxt_config(html: str) -> Tuple[Optional[str], Optional[str], Optional[str]]:
    m = NUXT_CONFIG_RE.search(html)
    if not m:
        return (None, None, None)
    body = m.group("body")
    m2 = NUXT_APP_RE.search(body)
    if not m2:
        return (None, None, None)
    return (m2.group("baseURL"), m2.group("buildId"), m2.group("buildAssetsDir"))
...
def discover_assets_and_payload(base_url: str, session: Optional[requests.Session] = None) -> Tuple[List[str], Optional[str]]:
    ...
    js_urls: set[str] = set()
    baseURL, buildId, buildAssetsDir = _parse_nuxt_config(html)
...
def extract_all_cards(base_url: str, workers: int = 8) -> List[CardRow]:
    ...
    for url, js in fetched.items():
        ...
        # ヒューリスティクス（最終手段）は JS が巨大な場合に重いので、発見ゼロの時だけ実行
        if not any_found:
...
def main():
    ...
    parser.add_argument("--base-url", default="https://asmape0104.github.io/scshow-calculator/",
                        help="スクショウ計算機のベースURL（末尾スラッシュ推奨）")
